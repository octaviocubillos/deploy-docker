...
# ==============================================================================
# Script para generar el archivo README.md completo para el proyecto.
#
# Este script crea el archivo desde cero, incluyendo toda la documentación
# y reemplazando placeholders como la versión del proyecto.
# ==============================================================================
...
if [ ! -f "$PACKAGE_FILE" ]; then
    echo "❌ Error: No se encontró el archivo '$PACKAGE_FILE' en el directorio actual."
    exit 1
fi

# --- Extracción de la Versión y Usuario ---
VERSION=$(grep -m 1 '"version":' "$PACKAGE_FILE" | cut -d'"' -f4)
NPM_USER=$(grep -m 1 '"name":' "$PACKAGE_FILE" | cut -d'"' -f4 | cut -d'/' -f1)

if [ -z "$VERSION" ]; then
    echo "❌ Error: No se pudo extraer la versión desde '$PACKAGE_FILE'."
    exit 1
fi

echo "ℹ️  Generando README.md para la versión $VERSION..."

# --- Creación del Contenido del README ---
# Se utiliza un Heredoc (Here Document) para escribir el contenido completo.
cat > "$README_FILE" <<EOF
# Herramienta de Despliegue Docker (deploy-docker)

![Versión](https://img.shields.io/badge/version-$PLACEHOLDER-blue.svg)
![Licencia](https://img.shields.io/npm/l/$NPM_USER/deploy-docker)
...
Puedes instalar la herramienta globalmente para uso general o localmente como una dependencia de desarrollo para mayor consistencia en equipos. La versión actual es **$PLACEHOLDER**.

**Global:**
\`\`\`bash
npm install -g $NPM_USER/deploy-docker@$PLACEHOLDER
\`\`\`

**Local:**
\`\`\`bash
npm install $NPM_USER/deploy-docker@$PLACEHOLDER --save-dev
\`\`\`
...
# --- Reemplazo Final de la Versión ---
# Usamos sed para reemplazar el placeholder con la versión real.
sed -i.bak "s/$PLACEHOLDER/$VERSION/g" "$README_FILE"
rm "${README_FILE}.bak" # Eliminar el archivo de backup

echo "✅ ¡Éxito! El archivo '$README_FILE' ha sido generado/actualizado a la versión $VERSION."
